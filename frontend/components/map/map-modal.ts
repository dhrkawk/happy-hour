...
export function createStoreOverlayElement(store: StoreListItemVM) {
  ...
  if (store.partershipText) {
    ...
    badge.style.cssText = `
      ...
      padding:1px 4px; border:1px solid ${c.blue200};
      background:${c.blue50}; color:${c.blue600};
      border-radius:5px; font-size:9px; font-weight:600; line-height:1;
    `;
  ...
  badgesRow.style.cssText = "display:flex; gap:4px; flex-wrap:wrap;";
  const evt = document.createElement("span");
  if (hasEvent) {
    evt.innerHTML = `<span style="display:inline-block;width:5px;height:5px;border-radius:9999px;background:${c.green700};"></span>이벤트 진행중`;
    evt.style.cssText = `
    ...
    evt.innerHTML = `<span style="display:inline-block;width:5px;height:5px;border-radius:9999px;background:#9ca3af;"></span>이벤트 준비중`;
    evt.style.cssText = `
      display:inline-flex; align-items:center; gap:4px;
      padding:1px 6px; border-radius:9999px;
      font-size:10px; font-weight:600;
  ...
  btnRow.style.cssText = "display:flex; justify-content:space-between; gap:6px; margin-top:6px;";
